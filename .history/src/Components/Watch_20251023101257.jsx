...
export default function Watch() {
        ...
        if (user) {
          ...
          if (user) {
            const favRes = await axios.get(
              `http://localhost:5000/api/favorite/check?user_id=${user.id}&movie_id=${id}`
            );
  ...
  const toggleFavorite = async () => {
    try {
      // 🧩 Đảm bảo movie đã được load xong
      if (!movie || !movie.slug) {
        console.warn("⚠️ Movie chưa sẵn sàng hoặc thiếu slug:", movie);
        return alert("Đang tải thông tin phim, vui lòng thử lại sau!");
      }

      // 🧩 Log để kiểm tra
      console.log("⭐ Gửi dữ liệu:", {
        user_id: user.id,
        movie_id: id, // slug phim trong URL
        slug: movie.slug, // slug từ API (đảm bảo không null)
      });

      // 🧩 Gửi request
      const res = await axios.post("http://localhost:5000/api/favorite", {
        user_id: user.id,
        movie_id: id,
        slug: id,
      });
      ...
      if (res.data.message.includes("thêm")) setIsFavorite(true);
      else if (res.data.message.includes("xóa")) setIsFavorite(false);
